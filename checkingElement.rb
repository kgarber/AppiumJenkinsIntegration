def textElement(findText,sendingText)
...
#appium.driver.find_element :uiautomator, args123233333
...
# This is a workaround for Appium issue 4311, 
# It can and should be replaced by the following when the issue is resolved1231233
# appium.driver.scroll_to textd
#text = %Q("#{text}")
#args = appium.scroll_uiselector("new UiSelector().text(#{text})")
...
def selectElement(findText,pickOption)
...
def picklistElement(findText,pickOption)
	puts "function started"
	while exists {text(findText)} == false 
		puts "after while"# && exists {text(pickOption)} == false
@driver.swipe(:start_x => 0, :start_y =>1000, :end_x => 0, :end_y => 50, :duration =>2000)
puts exists {text(findText)}
end
@driver.find_element(:name, findText).click()
